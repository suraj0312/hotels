...
def _payment_model_from_session() -> Any:
    """
    Return a value suitable for CheckoutUpdateRequest.payment.

    - If the SDK exposes a `payment_update_req.PaymentUpdateRequest` type,
      we try to build and return that instance.
    - Otherwise, we return a plain dict (or convert an existing model to dict).
    CheckoutUpdateRequest accepts either a dict or a PaymentUpdateRequest instance.
    """
    raw = _payment_from_session()

    # If the session already contains a dict, just return it.
    if isinstance(raw, dict):
        return raw

    # If it's already a model instance, try to convert to dict.
    ...
    # Try to construct PaymentUpdateRequest if available in the SDK.
    try:

        # Try to validate/construct a PaymentUpdateRequest from whatever we have.
        # If 'raw' is a dict-like object, model_validate will handle it.
        return payment_update_req.PaymentUpdateRequest.model_validate(raw)
    except Exception:
...
ucp_agent = Agent(
    ...
    instruction=(
        "You are a shopping assistant for a UCP merchant.\n\n"
        "Your job is to help the user manage a cart and checkout using the tools.\n"
        "You can understand simple natural language requests like:\n"
        "- 'what payment methods do you support?'\n"
        "- 'start a cart with 1 red rose'\n"
        "- 'add 2 ceramic pots'\n"
        "- 'set red rose quantity to 3'\n"
        "- 'remove ceramic pot'\n"
        "- 'apply discount 10OFF'\n"
        "- 'choose shipping'\n"
        "- 'pay using mock_payment_handler'\n\n"
        "Behavior rules:\n"
        "- If the user asks about merchant capabilities or payment methods, call discover_merchant or list_payment_handlers.\n"
        "- If the user asks to add/update items but no checkout exists, call start_checkout first.\n"
        "- After any cart change, call show_cart (or return cart summary) so the user sees items + total.\n"
        "- If a tool returns status=error, stop and explain what failed and what to do next.\n"
    ),
    tools=[
        discover_merchant,
        list_payment_handlers,
        list_all_items,
        start_checkout,
        show_cart,
        add_item,
        ...
        apply_discount,
        start_shipping_and_choose_first_option,
        complete_payment,
    ],
)
